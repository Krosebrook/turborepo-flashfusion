...
class UnifiedDatabaseService {
  constructor() {
    ...
    this.errorHandler =
      ErrorHandler.createServiceErrorHandler('UnifiedDatabase');
  }
  ...
  async initialize() {
    return this.errorHandler.execute(async () => {
      ...
      switch (this.dbType) {
        ...
        default:
          logger.warn(
            'No database configuration found - running in offline mode'
          );
          return false;
      }
    ...
    return this.errorHandler.execute(async () => {
      ...
      if (this.dbType === 'postgresql' && this.pgPool) {
        ...
        throw this.errorHandler.createError(
          'Query method not available for current database type',
          501
        );
      }
    }, 'query');
    ...
    return this.errorHandler.execute(async () => {
      if (this.dbType === 'supabase') {
        ...
        const whereClause = filterColumns
          .map((col, i) => `${col} = $${updateValues.length + i + 1}`)
          .join(' AND ');
