...
const checkNodeVersion = () => {
  ...
  if (currentVersion < REQUIRED_NODE_VERSION) {
    log.error(
      `Node.js ${REQUIRED_NODE_VERSION}+ required. Current: ${currentVersion}`
    );
    process.exit(1);
  }
};
...
`;
  ...
  upgrade: () => {
    ...
    try {
      ...
      log.warn(
        'npm-check-updates not available. Install with: npm i -g npm-check-updates'
      );
    }
  },
  ...
  'github:discover': async () => {
    ...
    if (!githubToken) {
      log.warn(
        'GitHub token not found in .env file. Using unauthenticated requests (limited rate).'
      );
    ...
    try {
      ...
      if (result.success) {
        ...
        topRepos.forEach((repo, index) => {
          console.log(
            `   ${index + 1}. ${repo.repo_name} (${repo.stars} ⭐) - ${repo.primary_language}`
          );
        });
      } else {
...
${colors.bright}📋 Documentation${colors.reset}
  ...
  ff docs checklist          View migration checklist
      ...
      case 'all':
        try {
          ...
          execSync(`./tools/migrate-repositories.sh ${subcommand}`, {
            stdio: 'inherit',
          });
        } catch (error) {
  ...
  validate: () => {
    ...
    try {
      ...
      execSync(`./tools/validate-migration.sh ${subcommand}`, {
        stdio: 'inherit',
      });
    } catch (error) {
    ...
    switch (subcommand) {
      case 'migration':
        if (fs.existsSync('docs/MONOREPO-INTEGRATION-PLAN.md')) {
          const { execSync } = require('child_process');
          try {
            execSync('cat docs/MONOREPO-INTEGRATION-PLAN.md | head -50', {
              stdio: 'inherit',
            });
            console.log(
      ...
      case 'checklist':
        if (fs.existsSync('docs/MIGRATION-CHECKLIST.md')) {
          const { execSync } = require('child_process');
          try {
            execSync('cat docs/MIGRATION-CHECKLIST.md | head -50', {
              stdio: 'inherit',
            });
          ...
          log.error(
            'Migration checklist not found. Run migration setup first.'
          );
        }
        break;
      default:
...
${colors.bright}🔧 Last Updated: ${new Date().toLocaleString()}${colors.reset}
  ...
  'ol:init': async () => {
    ...
    try {
      ...
      log.info(
        '💡 Available commands: ol:create, ol:generate, ol:status, ol:list'
      );
    } catch (error) {
  ...
  'ol:create': async () => {
    ...
    if (!name) {
      log.error(
        'Project name required: ff ol:create <name> [template] [description]'
      );
      return;
    }
    ...
    try {
      ...
      if (result.success && result.sandboxes.length > 0) {
        ...
        result.sandboxes.forEach((sandbox, index) => {
          ...
          console.log(
            `${colors.cyan}${index + 1}.${colors.reset} ${sandbox.id}`
          );
          console.log(
            `   ${colors.yellow}Template:${colors.reset} ${sandbox.template}`
          );
          console.log(
            `   ${colors.yellow}Status:${colors.reset} ${sandbox.status}`
          );
